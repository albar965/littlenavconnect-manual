...
for LANGNAME in "$@" ; do
  ...
  ebook-convert "${DEPLOY}/littlenavconnect_book_${LANGNAME}.epub" "${DEPLOY}/littlenavconnect_book_${LANGNAME}_a4.pdf" --preserve-cover-aspect-ratio --paper-size a4 --base-font-size 6

  # Create US Letter PDF
  echo
  echo =====================================================================================
  echo Running ebook-convert for US Letter PDF =============================================
  ebook-convert "${DEPLOY}/littlenavconnect_book_${LANGNAME}.epub" "${DEPLOY}/littlenavconnect_book_${LANGNAME}_letter.pdf" --preserve-cover-aspect-ratio --paper-size letter --base-font-size 6
